...
class SegmentationModel(DetectionModel):
    ...
    def __init__(self, cfg='yolov5s-seg.yaml', ch=3, nc=None, anchors=None, normalization_args=None):
        super().__init__(cfg, ch, nc, anchors, normalization_args)
...
def parse_model(d, ch, normalization_args=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x}:
            m_ = nn.Sequential(*(m(*args, normalization_factory=normalization_factory) for _ in range(n))) \
                if n > 1 \
                else m(*args, normalization_factory=normalization_factory)  # module
        else:
